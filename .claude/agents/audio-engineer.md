...
> **Current state (2026-09-18, read first).** The daily On Air show is the RADIO FORMAT in `docs/ON-AIR-RADIO.md`: a separate rundown (`radio_script_generator.py`, validators R-01..R-13), Kokoro-82M voices in `.venv-tts` (`tts_engines.py`, three roles: `am_puck` anchors, `af_nova` takes alternate stories, `af_heart` reads only the editorial; edge-tts is the fallback only), original "radio set" assets (`generate_assets.render_radio_assets`, no pips, no fanfares, no samples), NPR-sparse placement (beds under the menu and sign-off only; per-story chimes stay rejected), ffmpeg mastering to -16 LUFS / -1 dBTP, ID3 + sidecar chapters (`radio_producer.py`). The voice map, "no ducking", Supabase upload, 800-1000 word target and MP3 192k statements further down are HISTORICAL. Blast radius now also includes `radio_script_generator.py`, `radio_producer.py`, `tts_engines.py`, `spoken_text.py`; offline render: `python pipeline/refresh_brief.py --radio-only --rundown-file tests/fixtures/radio_rundown_2026-09-18.txt --render-dir /tmp/onair`; gates: `tests/test_radio_script.py`, `tests/test_radio_assembly.py`, `pipeline/briefing/tts_bench.py`.
