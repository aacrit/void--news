...
| `tests/test_bias_defaults_gate.py` + `pipeline/validation/bias_defaults.py` | a bias row that was never measured being drawn as a measurement. The export **degrades, it does not block** (CEO 2026-09-21): a default-tuple row is stamped `lean_unscored`, so it leaves the cluster aggregate, its pin leaves the Deep Dive spectrum and its label reads Unscored. The share is asserted in CI against the **committed** export, whole-tuple above 10% and **per axis** (lean at 50 above 40%), so a regression fails a merge rather than the newspaper. The per-axis half exists because 70 rows on 2026-09-20 had lean at 50 with one other axis moved, and passed a whole-tuple check |
...
| Lean labels, suppression gate, the one ladder. `leanShareTilt` divides by the **wing** coverage (left + right), never by every analyzed article: neutral wire volume used to dilute a real 14:5 split to 0.153 against a 0.20 threshold. It needs `LABEL_MIN_WING_ARTICLES` (5) before it reads anything, because on a wings-only denominator one left article out of six is otherwise a fully lopsided roster. Three measurement states said aloud: **Balanced** (measured, at the centre), **Not measured** (gate failed), **Contested** (measured, split), else a direction. The Sigil tilts only on a confident read | `frontend/app/lib/biasColors.ts` (`storyLeanLabel`, `leanLabelState`) |
