...
| **History** | Live, 78 events, static JSON since rev 69. Landing prerendered with an `<h1>` and every card (2026-09-21); era, region and thread browse routes linked and in the sitemap; native scroll, no wheel hijack. 149 time-bound claims dated or dropped 2026-09-21 (`docs/audits/HISTORY-DATA-2026-09-21.md`). **Thesis pilot** (2026-09-24): the Srebrenica ledger and thesis are committed at `draft` and pass every T-check; publication waits on the Stage 5 audit stamp (`audited_by`, `audited_at`) that T-13 requires. Partition and Mongol Baghdad not started. |
