...
Last updated: 2026-05-31 (rev 45, engine simplification unwinds the May 15 3-phase rewrite's over-engineering. **Phase 2 reverted to set-intersection** (commit `5402f7d` shape): `min_shared_entities=3`, 48h time gate, working union-find with `group_size[rb] += group_size[ra]` accumulation and `max_cluster_articles=50` hard cap. **Deleted**: `_LOW_SPECIFICITY_ENTITIES` downweight, `distinguishing_shared >= 1` gate, fallback IDF-weighted Jaccard branch, broken `_would_exceed_ceiling()` (used stale clusters[find(i)]). Moved generic government venues + wire-service names from `_LOW_SPECIFICITY_ENTITIES` into `_ENTITY_STOP_TOKENS` (so single-article cohorts don't false-merge on shared {White House, Treasury, Bessent}, but country/politician names stay distinguishing). **Phase 1 synonyms** (DRC↔Congo, UK↔Britain, "Donald Trump"↔Trump) normalised in `_build_document()` before TF-IDF, subsumes old Phase 2.55. **Phase 2.5** (canonical pairs) and **Phase 2.55** (synonym merge) removed from `cluster_stories()` pipeline. **Phase 2.6** anchor merge behind `enable_anchor_merge: bool = False` flag, diagnostic-only. **Phase 3** title-Jaccard threshold lowered 0.27 → 0.22 to match anchor floor + rescue multi-desk title diversity (Trump-Iran case). **Phase 5** simplified to 15-line soft-cap-only: at sc≥75 cap + stamp `mega_cluster_capped=True`, no force-split, no cohesion gate, no recursive sub-cap, no chain-merge-dissolve. **`is_headline`** rebuilt as `sc >= 8 AND headline_rank >= 40 AND not mega_capped` — no three-tier corpus-volume scaling, no Hill-curve src_pts weighted blend. `corpus_size` accepted as kwarg for backwards compat but ignored. **Engine snapshot writer** in `main.py` step 8c.6 DISABLED in production (saves 5-7 MB JSONB per run); `pipeline/engine_snapshot.py` and migrations 057-058 kept for opt-in sandbox use. **`diag.html`** marked deprecated with banner pointing to `/engine-plan.html`. Migrations 054-059 schema unchanged — no DDL rollback. Clustering suite: 33 → 38 fixtures (added `036-trump-iran-low-specificity-merge.yaml`, `037-observer-publisher-bridge-reject.yaml`, `038-transitive-trump-bridge-reject.yaml`). 34 CORRECT / 2 ACCEPTABLE / 2 WRONG / 0 CATASTROPHIC. Trump-Iran regression case (was CATASTROPHIC pre-simplification, sc=2 across 4 fragments) → now CORRECT (1 cluster, sc=5). See `docs/ENGINE-BREAKAGE-ANALYSIS.md`, `docs/ENGINE-SIMPLIFICATION-FULL-SCOPE.md`, `docs/ENGINE-SIMPLIFICATION-PLAN.html`.)
