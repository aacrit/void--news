...
export async function issueCard(issue: WeeklyDigestData): Promise<ImageResponse> {
  ...
  const font = await displayFont(
    headline + kicker + "VOID NEWS Weekly" + TAGLINE + "0123456789"
  );
  const fonts = font
    ...
    (
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {/* The nameplate, in the same grammar as the page it links to (see
              components/SectionNameplate): the masterbrand roman, a hairline,
              the section in accent italic. It read "VOID WEEKLY" before, which
              is the lockup form reserved for a PRODUCT of Void, and a share
              card is often the first place someone meets the brand. */}
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            ...
            <div style={{ display: "flex", width: 1, height: 26, background: INK, opacity: 0.28 }} />
            <div
              style={{
                display: "flex",
                fontSize: 30,
